...
while [ True ]
 do
   ...
   2)
	...
	nohup sh ~/Download/kafka_2.11-0.8.2.1/bin/kafka-server-start.sh ~/Download/kafka_2.11-0.8.2.1/config/server-1.properties > kafka-1.log 2>&1 &
	nohup sh ~/Download/kafka_2.11-0.8.2.1/bin/kafka-server-start.sh ~/Download/kafka_2.11-0.8.2.1/config/server-2.properties > kafka-2.log 2>&1 &
	;;
   3)
	read -p "Topic名:" topic
   ...
   7)
	read -p "Topic名:" topic
	sh ~/Download/kafka_2.11-0.8.2.1/bin/kafka-topics.sh --describe --zookeeper localhost:2181 --topic $topic
	;;
   0)
	exit;
	;;
